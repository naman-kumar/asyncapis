...
class ConsoleHealthCheck:
    ...
    async def _processing(self, conn, request):
        await asyncio.sleep(random.randrange(5, 20))
        conn.write(request.encode('utf8'))
        await conn.drain()
        # return conn.sendall(request.encode('utf8'))

    async def run_health_check(self, conn, request):
        # ping_status = await self._ping_check()
        # ssh_status = await self._run_ssh_command('hostname', username='', password='')
        task_no = await self._processing(conn, request)
        print({
